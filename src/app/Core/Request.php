...
class Request
{
  ...
  public function __construct(
    ...
    // Check if type of $get_param objects are string
  ...
  private function isUriValid(string $uri): bool
  {
    if (preg_match(pattern: '/^([\/]?(.+)*(\?.+=.+)?)$/', subject: $uri)) {
      return true;
    } else if (preg_match(pattern: '{^([\/](/image)[\/].+)$}', subject: $uri)) {
      return true;
    } else {
      return false;
    }
  ...
  public function getMessageFromParam(): string
  {
    if (isset($this->get_param['message'])) {
      return $this->get_param['message'];
    } else {
      return '';
    }
  }
